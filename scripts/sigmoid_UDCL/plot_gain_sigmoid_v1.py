...
sigmoidUDCL = SigmoidUDCL(evol_pars, game_pars, calc_minus1_system=False, precalc_payoffs=False)
...
for strats in L:
    ...
    # strategy 1 payoffs 
    aV = [sigmoidUDCL.payoff(strats, strat_counts) for strat_counts in strat_countsV]

    # strategy 2 payoffs 
    r_strats = list(reversed(strats))
    bV = [sigmoidUDCL.payoff(r_strats, strat_counts) for strat_counts in reversed(strat_countsV)]
    ...
    plt.tight_layout()
    plt.savefig(savedir + 'gain_sigmoid_v1_' + '_Vs_'.join(strats) + '.pdf')
    plt.close('all')
